...
def pool_forward(A_prev, stride,f):
...
def layer_forward(A_prev, W, b, activation, mode):
    '''
    ...
    drop_mask = np.random.choice([0,1], Z.shape[0])
    drop_mask = drop_mask.reshape((drop_mask.shape[0],1))
    if activation == "relu":
        A, act_cache = relu(Z)
...
def multi_layer_forward(X, parameters, mode):
    '''
    ...
    for l in range(1, L):  # since there is no W0 and b0
        A, cache = layer_forward(A, parameters["W" + str(l)], parameters["b" + str(l)], "relu",mode)
        caches.append(cache)

    AL, cache = layer_forward(A, parameters["W" + str(L)], parameters["b" + str(L)], "linear",mode)
    caches.append(cache)
    return AL, caches
...
def classify(X, parameters,conv_parameters):
    ...
    A1,cache_pool = pool_forward(A1,1,2)
    A1 = np.reshape(A1, (A1.shape[1] * A1.shape[2] * A1.shape[3] , A1.shape[0]))
    A, c1 = multi_layer_forward(A1, parameters,"testing")
...
def multi_layer_network(X, Y, net_dims, num_iterations=500, learning_rate=0.2, decay_rate=0.01):
    ...
    for ii in range(num_iterations):
        ...
        A2,maxpool_cache = pool_forward(A1,1,2)
        A1 = np.reshape(A2, (A2.shape[1] * A2.shape[2] * A2.shape[3], A2.shape[0]))
        # print A1.shape
        ...
        Z, caches = multi_layer_forward(A1, parameters,"training")        ## call to multi_layer_forward to get activations
        ...
        dA = pool_backward(dA,maxpool_cache)
